...
provide-module snippets %{
  ...
  define-command snippets-enable -docstring 'Enable snippets' %{
    map global insert <C-e> '<esc>: type-expand-command snippets-expand-selection<ret>' -docstring 'Expand the currently entered snippet (between a matching pair)'
  }

  define-command snippets-disable -docstring 'Disable snippets' %{
    unmap global insert <C-e>
  }
  ...
  define-command -hidden snippets-paste -params 2 %{
    ...
    try %{
      ...
      map window insert <c-n> '<c-;>: snippets-select-next-placeholder<ret>' -docstring 'Select the next placeholder'
      map window insert <c-p> '<c-;>: snippets-select-previous-placeholder<ret>' -docstring 'Select the previous placeholder'
  ...
  define-command -hidden snippets-select-placeholder -params 1 %{
    try %{
      ...
      evaluate-commands -itersel %{
        execute-keys "<c-;>s<ret><c-;>%arg{1}<c-;><space><c-;>d"
      }
    }
  }
  ...
  define-command -hidden snippets-paste-text -params 2 %{
    evaluate-commands -save-regs '"' %{
      # Paste using the specified method
      # The command (R, <c-P> and <c-p>) selects inserted text
      set-register '"' %arg{2}
      execute-keys %arg{1}
      # Remove EOF newline
      ...
      try %{
        ...
        execute-keys -draft "<c-s>s\A\t+<ret>s.<ret>%opt{indentwidth}@"
      }
      # Align everything with the current line
      evaluate-commands -draft -itersel %{
        try %{
          execute-keys '<c-s>Z)<space><c-x>s^\h+<ret>yz)<c-space>_P'
        }
      }
    }
  }
